...
public class BOJ16959_체스판_여행_1 {
    ...
    static int solve() {
        ...
        int ret = 0;
        for (int i = 0; i < 3; i++) {
            Point start = new Point(points[1].r, points[1].c, 1, i, 0);
            que.add(start);
            visited[start.r][start.c][start.num][start.piece] = true;
        }
        ...
        while (!que.isEmpty()) {
            ...
            for (int i = 0; i < nextSamePiece.size(); i++) {
                ...
                if (!visited[next.r][next.c][next.num][next.piece]) {
                    visited[next.r][next.c][next.num][next.piece] = true;
                    que.add(next);
                }
            }
    ...
    static class Point implements Comparable<Point> {
        ...
        public Point(int r, int c, int num, int piece, int cnt) {
            this.r = r;
            this.c = c;
            this.num = num;
            this.piece = piece;
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = -1; j <= 1; j += 2) {
                    ...
                    if (checkBoundary(nr, nc)) {
                        ...
                        ret.add(new Point(nr, nc, nn, this.piece, this.cnt + 1));
                    }
                }
            }
            return ret;
        ...
        public List<Point> changePiece() {
            ...
            for (int i = 1; i <= 2; i++) {
                int np = (piece + i) % 3;
                ret.add(new Point(r, c, num, np, cnt + 1));
            }
            return ret;
        }
